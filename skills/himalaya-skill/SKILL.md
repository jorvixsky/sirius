...
**Sent-copy requirement:** For SMTP sends to appear in webmail/SOGo Sent, the account config must include `message.send.save-copy = true` and a valid `folder.aliases.sent = "Sent"` (or the provider's real Sent folder). Do not use the obsolete top-level `save-sent`; do not map `sent` to `INBOX` except as a deliberate emergency workaround.
